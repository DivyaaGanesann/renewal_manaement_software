...
if (isset($_GET['delete'])) {
    ...
    if ($id > 0) {
        $stmt = $conn->prepare("DELETE FROM map_configuration WHERE id=?");
        $stmt->bind_param("i", $id);
        $stmt->execute();
        $_SESSION['msg'] = "MAP Configuration Deleted Successfully!";
...
/* ---------- FILTER ---------- */
$where = "WHERE YEAR(m.created_at) = YEAR(CURDATE())"; // Only this year
$filters = [];
...
<h3 class="text-center mb-3">MAP Configuration List (This Year)</h3>
...
<!-- FILTERS -->
<div class="dr-filter-box" style="display:flex; gap:15px; flex-wrap:wrap; align-items:flex-end; margin-bottom:20px;">
...
<!-- TABLE -->
<table id="mapTable" class="table table-bordered table-striped table-sm mt-3">
<thead>
<tr>
    <th>SNO</th>
    <th>Customer Name</th>
...
<tr>
    ...
    <td class="text-center">
        <a href="map_view.php?id=<?= $id ?>"><i class="fa fa-eye text-success"></i></a>
        <a href="map.php?id=<?= $id ?>" class="ms-2"><i class="fa fa-edit text-primary"></i></a>
        <a href="map_list.php?delete=<?= $id ?>" class="ms-2" onclick="return confirm('Delete this MAP?')">
        ...
        drawCallback:function(){
            $('#mapTable tbody tr').each(function(){
                $(this).find('td').each(function(index){
                    if(index === 8) return; // skip Actions
                    if ($(this).find('span,i,a,button').length) return;
