...
if (isset($_GET['delete'])) {
    $delete_id = intval(base_dcode64($_GET['delete']));
    if ($delete_id > 0) {
        $stmt = $conn->prepare("DELETE FROM seo_details WHERE id=?");
        $stmt->bind_param("i", $delete_id);
        if ($stmt->execute()) {
            $_SESSION['msg'] = "SEO Record Deleted Successfully!";
...
/* ---------- FILTER PROCESS ---------- */
$where = "WHERE YEAR(s.created_at) = YEAR(CURDATE())"; // Only current year
$filters = [];
...
if (isset($_SESSION['msg'])) {
    ...
    $session_msg_type = $_SESSION['msg_type'];
    unset($_SESSION['msg']);
...
<h3 class="text-center mb-3">SEO Report (This Year)</h3>

<?php if($session_msg): ?>
<div class="alert alert-<?=$session_msg_type?> text-center"><?=$session_msg?></div>
<?php endif; ?>
...
<table id="seoTable" class="table table-bordered table-striped table-sm">
<thead>
<tr>
    <th>SNO</th>
    <th>Customer</th>
...
<tr>
    ...
    <td class="text-center">
        <a href="seo_view.php?id=<?=$id?>"><i class="fa fa-eye text-success"></i></a>
        <a href="seo.php?id=<?=$id?>" class="ms-2"><i class="fa fa-edit text-primary"></i></a>
        <a href="seo_list.php?delete=<?=$id?>" class="ms-2" onclick="return confirm('Are you sure you want to delete this SEO record?')">
